...
When you draw on knowledge of the dungeon, or wake after a \smallcaps{long rest}, then say the thing you know because of your role or job in the dungeon. You get that statement free. The GM needs to do their best to make it true. Now \smallcaps{roll + identity}
\begin{itemize}
...
\begin{itemize}
\item On a 7+ you steal their shadow\index[stuff]{shadow} and hold it indefinitely.
...
\item On a 7-9 choose two:
	...
	\myitem You don't heal them one health at the cost of two damage to yourself.
	\end{itemize}
\end{itemize}
...
\item Choose two different rows:
	...
	\myitem Undead Pet \smallcaps(+1 Thief)
	\end{itemize}
\item Choose a tag from each row 
	\begin{itemize}
	\myitem Religious; Bone; Compact; Fine
	\myitem Cursed; Golden; Large; Ancient
	\end{itemize}
\end{itemize}
...
\item On a 7-9 Get two hold. Spend hold one for one to:
	...
	\myitem Hold a room fast with rotting limbs for a scene
	\end{itemize}
...
\item On a 7-9 Get two hold. Spend hold one for one to:
	...
	\myitem you don't desperately craze blood, brains or flesh. Otherwise you must satisfy this hunger before you can make this move again.
	\end{itemize}
...
It's a big ole dungeon, and a short fast life. Wanna buy some specs\index[stuff]{spectacles} to watch it go by?
...
Members of the Horde are rarely alone. At anyone one time there's a member of the horde present for every two health points you have (rounding up). When you take two damage, someone just died, hopefully both gruesomly and entertainingly. When you heal two health points, make a note and have a cousin or neice of the current Face of the Horde turn up out of a chimney, toilet, locked box or similar when convenient.

\section{Maslov's Challenge}\index{Maslov's Challenge: move}
At the start of the session \smallcaps{roll+thief} on behalf of the wider horde:
\begin{itemize}
\item whatever the challenge, if you help gain Xp. 
\item On a 7+ gain a reroll in your pocket for this session.
\item On a 10+, choose one challenge:
	\begin{itemize}
	\myitem a wedding
	\myitem a trade war
	\myitem a lost child
	\end{itemize}
\item On a 7-9 choose one problem:
	\begin{itemize}
	\myitem a predator
	\myitem bandits
	\myitem a flood
...
\item On a 6- choose one desperate need:
	...
	\myitem food
	\myitem safe rooms
	\myitem medicine \index[stuff]{medicine}
	\end{itemize}
\end{itemize}
...
\item On a 10+ choose two bonuses:
	...
	\myitem They'll stick around for a bit, even if there's trouble
	\end{itemize}
\item On a 7-9 choose a bonus from above and complication from below
	...
	\myitem They are undisciplined kleptomaniacs
	\end{itemize}
...
\item On a 10+ choose three:
	...
	\myitem the trap can be dismantled for a useful part
	\end{itemize}
\item On a 7-9 choose two
...
\item On a 10+ ask the GM three:
	...
	\myitem What do they have of value?
	\end{itemize}
...
\item On a 10+ choose three:
	...
	\myitem They don't have a grudge against a player character
	\end{itemize}
\item On a 7-9 choose two
...
\marginnote{Name: Aaargh; the great clawed beast; smell-in-the-dark; white-wyrm-o-the-well; Gulp \bigskip}
\marginnote{Appearance (choose 1-3): extra legs, albino, bedraggled, too many eyes\bigskip}
...
You have a movement skill suited to your environment (choose one)
\begin{itemize}
\item flyer
\item climber
\item swimmer and diver
\item burrower
\item leaper
\end{itemize}
...
\item On a 10+ get all three
	...
	\myitem You will be the cat this time
	\end{itemize}
\item On a 7-9 choose one
...
\item On a 10+ choose two:
	\begin{itemize}
	\myitem You are impossible to spot as different (otherwise you leave a telltale)
	\myitem You can transform inn the blink of an eye (otherwise it takes some time)
	\myitem You can go up or down a size when you transform (otherwise you remain the same size and weight)
	\end{itemize}
\item On a 7-9 choose one.
...
\item spend hold 1 for 1 as required for a scene
	...
	\myitem \smallcaps{+1 thief} Consume fabrics, inks or poisons \index[stuff]{fabric} \index[stuff]{ink} \index[stuff]{poions}
	\end{itemize}
\end{itemize}
...
\item On a 10+ you get all three:
	\begin{itemize}
	\myitem Another player character can ask what the the NPC wants and you can give an detailed answer \marginnote{Collaborate with the GM, but the answer should be accurate}
	\myitem You can stop the simulation at will (otherwise it takes a bit of time or might come back)
	\myitem The simulation doesn't act on it's own desires 
	\end{itemize}
...
You have a sanctum, a workshop, library or pocket universe. Choose four good things and two bad. \marginnote{ Designer's note: These are all about fictional positioning, magical equivalents of announcing you climb the wall using crampons and a rope compared to just climbing it. Rule 4 suggests it should be more explicit then that. Needs testing.}
\begin{itemize}
\item Good things
	\begin{itemize}
...
When you and your gang set an ambush on the road \smallcaps{roll+thief} 
\marginnote{designer's note. This is very stat heavy fiction low. Maybe revise?}
\begin{itemize}
\item On a 10+ your preparations are clever. Take \smallcaps{+1 theif} into the scene.
...
When you have indulged in your chosen vice heavily (what?) and you fail a roll, accept the effects of the fail, but you can try to succeed on the roll one more time before the story moves on. This means you can have a 10+ result and a 6- result at the same time.
...
\marginnote{designer's note: does this contradict the principle of clear info? It's even more pressure on GM too, not just to come up with the answer but 'translate' it into something like snake eyes, or the dice look like a wizards hat ect... needs testing}

\begin{itemize}
\item On a 10+ ask three questions. The GM will answer, but framed in the cards, dice results or other suitable: 
	\begin{itemize}
